...
(
    ...
    property_id integer not null
        constraint material_property_property_fk
            references property,
    constraint material_property_uid_property_id_key
...
do $$
begin
    if to_regprocedure('stamp_updated()') is null then
    ...
    if not exists (
        select 1
        from pg_trigger
        where tgname = 'material_value_stamp_updated'
          and tgrelid = 'material_value'::regclass
...
drop table if exists material_value cascade;
drop table if exists material_property cascade;
drop table if exists material_source cascade;
drop table if exists delivery_type cascade;
drop table if exists unit cascade;
drop table if exists material_group cascade;
